...
@Component
public class EmployeeLaborClassificationLookupAdapter implements EmployeeLaborClassificationLookupPort {

    private final EntityManager entityManager;

    public EmployeeLaborClassificationLookupAdapter(EntityManager entityManager) {
    ...
    private Optional<EmployeeLaborClassificationContext> mapResult(List<?> rows) {
        if (rows.isEmpty()) {
        ...
        if (!(row instanceof Object[] columns) || columns.length < 4) {
            throw new IllegalStateException("Unexpected row shape for labor classification employee lookup query");
        }
        ...
        return Optional.of(new EmployeeLaborClassificationContext(id, ruleSystemCode, employeeTypeCode, employeeNumber));
    }
}
