...
@Component
public class ContractPresenceConsistencyAdapter implements ContractPresenceConsistencyPort {

    private static final LocalDate MAX_DATE = LocalDate.of(9999, 12, 31);

    private final EntityManager entityManager;

    public ContractPresenceConsistencyAdapter(EntityManager entityManager) {
    ...
    private PresencePeriod toPresencePeriod(Object row) {
        if (!(row instanceof Object[] columns) || columns.length < 2) {
            throw new IllegalStateException("Unexpected row shape for contract presence query");
        }
    ...
    private LocalDate toLocalDate(Object value) {
        ...
        throw new IllegalStateException(
                "Unexpected date type in contract presence query: " + value.getClass()
        );
    }
}
